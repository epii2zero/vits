...
class SynthesizerTrn_VA(nn.Module):
  """
  ...
  def forward(self, x, x_lengths, y, y_lengths, sid=None, pitch_target=None, energy_target=None):
    # x [batch, max phoneme length]: text, y: spectrogram
    x, m_p, logs_p, x_mask = self.enc_p(x, x_lengths)
    # x [batch, embedding dim, max length]: text embedding
    # m_p & logs_p [batch, embedding dim, max length]: projected to statistics
    ...
    # [b, t', t], [b, t, d] -> [b, d, t']
    # VA
    stats_va, pitch_prediction, energy_prediction = self.VA(
      torch.cat([m_p, logs_p], 1), mel_mask=y_mask,
      pitch_target=pitch_target, energy_target=energy_target,
      train=True
    )
    m_p, logs_p = torch.split(stats_va, self.hidden_channels, dim=1)
    z_slice, ids_slice = commons.rand_slice_segments(z, y_lengths, self.segment_size)
    o = self.dec(z_slice, g=g)
    return o, l_length, attn, ids_slice, x_mask, y_mask, pitch_prediction, energy_prediction,\
           (z, z_p, m_p, logs_p, m_q, logs_q)
  

  def infer(self, x, x_lengths, sid=None, noise_scale=1, length_scale=1, noise_scale_w=1.,
            pitch_control=None, energy_control=None, max_len=None):
    x, m_p, logs_p, x_mask = self.enc_p(x, x_lengths)
    ...
    stats_va, pitch_prediction, energy_prediction = self.VA(
      torch.cat([m_p, logs_p], 1), mel_mask=y_mask,
      p_control=pitch_control, e_control=energy_control, train=False
    )
    m_p, logs_p = torch.split(stats_va, self.hidden_channels, dim=1)
    ...
    return o, attn, y_mask, (z, z_p, m_p, logs_p, pitch_prediction, energy_prediction)
